...
const ThreeDCharacter = ({ modelUrl }) => {
    ...
    const rotationFactor = useRef(0.001); // Reference to store the animation frame ID
    ...
    useEffect(() => {
        if (modelUrl) {
            ...
            const maxRotation = Math.PI/3; // 180 degrees in radians
            
            const animate = () => {
                if (modelRef.current) {
                    currentRotation += 0.001 * rotationDirection;
            
                    if (currentRotation >= maxRotation+ Math.PI/6 || currentRotation <= -maxRotation+ Math.PI/6) {
                        rotationDirection *= -1;  // Reverse the direction
                    }
            
             //       modelRef.current.rotation.y += 0.001 * rotationDirection;
                    modelRef.current.rotation.y += rotationFactor.current;
                }
                controls.update();
    ...
    return <div onMouseLeave={()=>{rotationFactor.current = 0.001}} onMouseEnter={()=>{rotationFactor.current = 0.01}} ref={mountRef} style={{ width: '100%', height: '100%' }} />;
};
