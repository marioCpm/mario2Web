...
export default function JourneyMap({ selectedJourney }) {
    const [unitHoverID, setUnitHoverID] = useState(-1)
    const { updateUnitWithProgress } = useMiddleware();
                ...
                <div className="container-fluid">
                    ...
                    <div className="row">
                        <div className="tf-heading mb60 wow fadeInUp">
                            <h2 className="heading">{selectedJourney?.topic}</h2>
                            <h6 className="heading">{selectedJourney?.description}</h6>
                        </div>
                        <div className="col-md-12 wow fadeInUp">
                            <div className="tf-roadmap">
                                <div className="swiper-container swiper sl-roadmap">
                                    <Swiper {...swiperOptions} className="swiper-wrapper">
                                        {selectedJourney?.units?.map((unit, index) => (
                                            <SwiperSlide>
                                                ...
                                                <div className="roadmap-box">
                                                    ...
                                                    <div onMouseLeave={() => setUnitHoverID(-1)} onMouseOver={() => setUnitHoverID(unit.id)} className="content" data-tooltip-id={`tooltip-${unit.id}`}>
                                                    <h4 className="title">{unit.topic}</h4>
                                                        <ul className="list-infor">
                                                            {unitHoverID == unit.id?<h6>{unit.description}</h6>:<> {unit?.concepts?.map((concept, index) => (
                                                                <li>
